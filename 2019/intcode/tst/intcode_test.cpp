...
void simpleAmplifierLoopTestHelper(std::string message,
                                   std::vector<long> input,
                                   long expectedOutput,
                                   std::vector<long> expectedPhases)
{
    ...
    do
    {
        ...
        {
            amplifiers.emplace_back("", input);
            amplifiers.back().pausable = true;
        ...
        {
            std::tie(output, state) = amplifiers[i].runningLoop(output);
...
TEST(IntCodeComputerTest, AmplificationLoopDay7) {
    simpleAmplifierLoopTestHelper(
